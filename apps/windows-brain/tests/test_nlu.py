...
class MockOrchestrator(LLMOrchestrator):
    def _generate_with_provider_llm(self, *args, **kwargs):
        ...
        elif "사업의 목적 및 배경은 글자 크기를 30pt로 만들고 그 밑 본문은 글자 크기를 15pt로 만들어" in prompt:
            return '{"intent": "style_update", "entities": {"raw": "사업의 목적 및 배경은 글자 크기를 30pt로 만들고 그 밑 본문은 글자 크기를 15pt로 만들어", "target_scope": "all"}, "actions": [{"type": "set_font_size", "value": "30", "target_scope": "사업의 목적 및 배경"}, {"type": "set_font_size", "value": "15", "target_scope": "본문"}]}'
        elif "첫 번째 문단은 맑은 고딕으로 하고 두 번째 문단은 굴림체로 바꿔" in prompt:
            return '{"intent": "style_update", "entities": {"raw": "첫 번째 문단은 맑은 고딕으로 하고 두 번째 문단은 굴림체로 바꿔", "target_scope": "all"}, "actions": [{"type": "set_font_family", "value": "Malgun Gothic", "target_scope": "첫 번째 문단"}, {"type": "set_font_family", "value": "Gulim", "target_scope": "두 번째 문단"}]}'
...
def test_nlu_parse_compound_requests():
    engine = NLUEngine()
    orchestrator = MockOrchestrator()
    provider = "mock"
    profile = {}
    
    # Test specific named sections
    r1 = engine.parse("사업의 목적 및 배경은 글자 크기를 30pt로 만들고 그 밑 본문은 글자 크기를 15pt로 만들어", orchestrator=orchestrator, provider=provider, auth_profile=profile, context=None)
    assert len(r1.actions) == 2
    act_1 = next(a for a in r1.actions if a.get("target_scope") == "사업의 목적 및 배경")
    act_2 = next(a for a in r1.actions if a.get("target_scope") == "본문")
    assert act_1.get("type") == "set_font_size" and act_1.get("value") == "30"
    assert act_2.get("type") == "set_font_size" and act_2.get("value") == "15"

    # Test distinct sequential sections
    r2 = engine.parse("첫 번째 문단은 맑은 고딕으로 하고 두 번째 문단은 굴림체로 바꿔", orchestrator=orchestrator, provider=provider, auth_profile=profile, context=None)
    assert len(r2.actions) == 2
    act_a = next(a for a in r2.actions if a.get("target_scope") == "첫 번째 문단")
    act_b = next(a for a in r2.actions if a.get("target_scope") == "두 번째 문단")
    assert act_a.get("type") == "set_font_family" and act_a.get("value") == "Malgun Gothic"
    assert act_b.get("type") == "set_font_family" and act_b.get("value") == "Gulim"


if __name__ == "__main__":
    test_nlu_parse_first_line()
    test_nlu_parse_all()
    test_nlu_parse_compound_requests()
